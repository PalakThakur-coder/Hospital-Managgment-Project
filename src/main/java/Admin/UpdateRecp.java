    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res)
	   ...
	   String responsibility = req.getParameter("responsibility");
        ...
        try {
            ...
            cn.DataConnection();
            PreparedStatement pst = cn.c.prepareStatement("UPDATE recptable SET name = ?, age = ?, gender = ?, qualification = ?, responsibility = ?, mobileno = ?, email = ?, houseno = ?, street = ?, city = ?, state = ?, date = ?, idname = ?, idno = ? WHERE id = ?");

            pst.setString(1, name);
            pst.setString(2, age);
            pst.setString(3, gender);
            pst.setString(4, qualification);
            pst.setString(5, responsibility);
            ...
            if (i > 0) {
                ...
                pw.println("window.location.href = \"Admin/ListRecp.jsp\";");
                pw.println("</script>");
